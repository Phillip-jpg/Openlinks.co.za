...
if($_GET['w'] == 1){
 ...
 $qry = $conn->query("SELECT DISTINCT CONCAT(u.firstname, ' ', u.lastname) AS fullname, 
       c.company_name, 
       pl.id, 
        ts.team_name,
...
LEFT JOIN users u1 ON
    ts.op_ids = u1.id
LEFT JOIN working_week_periods wwp ON ad.start_date BETWEEN wwp.start_week AND wwp.end_week
...
LEFT JOIN users u1 ON
    ts.op_ids = u1.id
JOIN working_week_periods wwp ON ad.end_date BETWEEN wwp.start_week AND wwp.end_week
WHERE wwp.period = $period
ORDER BY pl.id;
");
...
LEFT JOIN users u1 ON
    ts.op_ids = u1.id
JOIN working_week_periods wwp ON ad.Done_Date BETWEEN wwp.start_week AND wwp.end_week
WHERE wwp.period = $period
ORDER BY pl.id;
");
...
echo '<p style="font-weight:bold">Jobs Assgined this week</p>';
