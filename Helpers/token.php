...
//bl means business logic
    ...
    static function val_unauth($token, $bussiness_logic){
        ...
        // print_r($token);
        // exit();
        $calc =hash_hmac('sha256', $secret, $array[$token[0]]);
        if (hash_equals($calc, substr($token, 1))) {
            return "TRUE";
        }else {
            $calc =hash_hmac('sha256', $secret, $array[$token[0].$token[1]]);
            if(hash_equals($calc, substr($token, 2)))
            {
                return "TRUE";
            }
            else{
                $calc =hash_hmac('sha256', $secret, $array[$token[0].$token[1].$token[2]]);
