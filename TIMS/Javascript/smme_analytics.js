...
function entity_connections_chart(percentages){
    ...
    // #0dc0ff, #032033
 let chart = document.getElementById("smme_connections_chart").getContext("2d");//chart positioning in html
...
function search_chart(){
   ...
   // #0dc0ff, #032033
    let chart = document.getElementById("smme_search_chart").getContext("2d");//chart positioning in html
    ...
    data: {
        ...
        datasets: [{
            ...
            data: [25, 15, 50, 10],//the percentages for each smme(y axis)
...
function keywords_chart(percentages){
   ...
   // #0dc0ff, #032033
    let chart = document.getElementById("smme_keyword_chart").getContext("2d");//chart positioning in html
    let myChart = new Chart(chart, {
    type: "doughnut",
...
function ANALYTICS_SUMMARY_HEADER(){//display number
    ...
    let identifier = "SMME_ANALYTICS_HEADER";
    let display_id = "smme_analytics_header";
    make_request(identifier, token, display_id );
}
function KEYWORD_GRAPGH(){//chart
    let token = document.getElementById("tk").value;
    let identifier = "SMME_KEYWORD_GRAPGH";
...
function ANALYTICS_MARKETPLACE_HEADER(){//display number
    let token = document.getElementById("tk").value;
    let identifier = "SMME_MARKETPLACE_HEADER";
    let display_id = "smme_marketplace_header";
    make_request(identifier, token, display_id );
}

function CONNECTIONS_GRAPGH(){//chart
    let token = document.getElementById("tk").value;
    let identifier = "SMME_CONNECTIONS_GRAPGH";
    make_request(identifier, token, identifier, entity_connections_chart);
    
}
function SEARCH_CHART(){//chart
    let token = document.getElementById("tk").value;
    let identifier = "SMME_SEARCH_GRAPGH";
    let display_id = "smme_search_chart";
    make_request(identifier, token, identifier, search_chart);
    
}
function PROFILE_STATS(){//chart
    let token = document.getElementById("tk").value;
    let identifier = "SMME_PROFILE_STATS";
    let display_id = "smme_profile_stats";
    make_request(identifier, token, display_id );
